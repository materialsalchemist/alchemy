...
class ChemicalSpace:
	...
	def __post_init__(self):
		...
		self._allowed_valences = {symbol: tuple(pt.GetValenceList(symbol)) for symbol in self._heavy_atoms}
	...
	def explore(self):
		...
		self.build_cleavage_products_to_db(self.smiles, max_cuts=2, include_ring_bonds=True)
	...
	def _check_obabel_installed(self):
		...
		try:
			...
			click.secho(
				"Open Babel (obabel) is not installed or not found in PATH. Please install it to use this feature.",
				fg="red",
				err=True,
			)
	...
	def export_images(self):
		...
		for i, smi in enumerate(tqdm(sorted_smiles, desc="Exporting images")):
			...
			try:
				...
				if rdkit_mol:
					Draw.MolsToImage([rdkit_mol]).save(os.path.join(img_dir, f"{mol_filename_base}.png"))
	...
	def build_cleavage_products_to_db(
		...
		*,
		...
		if parent_smiles is None:
			click.secho("No parent SMILES provided for cleavage product generation.", fg="red")
			return 0
