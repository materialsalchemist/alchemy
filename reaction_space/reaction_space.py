...
class ReactionSpace:
    ...
    def __post_init__(self):
        ...
        self._db_paths = {
            ...
            "candidates_g2plus": os.path.join(
                db_dir, "reaction_candidates_g{gen}_c{max_c}.lmdb"
            ),
            "verified": os.path.join(db_dir, "verified_reactions.lmdb"),
        }
        logging.info(
            f"ReactionSpace initialized. Results will be saved in '{self.output_dir}'"
        )
    ...
    def _lmdb_batch_iterator(
        self, db_paths: List[str], batch_size: int, custom_reactants_filter: set = None
    ):
        """
                    ...
                    for key, value in cursor:
                        if key not in seen_keys:
                            ...
                            try:
                                ...
                                if (
                                    custom_reactants_filter
                                    and not __should_keep_reaction(
                                        smi, custom_reactants_filter
                                    )
                                ):
                                    continue
                                ...
                                logging.warning(
                                    f"Skipping malformed entry with key {key.hex()}: {e}"
                                )
                                continue
    ...
    def find_reaction_candidates(self):
        ...
        initial_molecules_set = {
            canonicalize_smiles(s)
            for s in initial_molecules
        ...
        if os.path.exists(g0_db_path):
            ...
            with Pool(self.n_workers) as pool:
                results_iterator = pool.imap(
                    get_dissociation_fragments, initial_molecules
                )
                ...
                for parent_smi, frag_set in tqdm(
                    ...
                    for f1, f2 in frag_set:
                        if not (
                            f1 in initial_molecules_set and f2 in initial_molecules_set
                        ):
                            continue
                        ...
                        if reaction_smi not in all_reactions_written and p == r:
                            ...
                            key = (
                                hashlib.sha256(reaction_smi.encode("utf-8"))
                                .hexdigest()
                                .encode("utf-8")
                            )
                            value = json.dumps(
                                {"smi": reaction_smi, "gen": "G0"}
                            ).encode("utf-8")
                            q.put((key, value))
        ...
        if self.custom_reactants_csv and os.path.exists(self.custom_reactants_csv):
            try:
                ...
                if "SMILES" in df_custom.columns and not df_custom.empty:
                    ...
                    click.secho(
                        f"Loaded {len(custom_molecules)} custom molecules from {self.custom_reactants_csv}",
                        fg="green",
                    )
                else:
                    click.secho(
                        f"'{self.custom_reactants_csv}' is empty or missing 'SMILES' column. Skipping G0.5.",
                        fg="yellow",
                    )
            except Exception as e:
                click.secho(
                    f"Could not read '{self.custom_reactants_csv}'. Skipping G0.5. Error: {e}",
                    fg="red",
                )

        if not custom_molecules:
            click.secho(
                "No custom molecules provided. Skipping G0.5 step.", fg="yellow"
            )
            g05_reactions = []
        elif os.path.exists(g05_db_path):
            click.secho(
                f"Skipping G0.5, database already exists at {g05_db_path}", fg="green"
            )
            ...
            with Pool(self.n_workers) as pool:
                results_iterator = pool.imap_unordered(
                    worker_func, reaction_pairs, chunksize=512
                )

                for res_list in tqdm(
                    results_iterator, total=len(reaction_pairs), desc=tqdm_desc
                ):
                    for candidate in res_list:
                        if candidate not in all_reactions_written:
                            ...
                            key = (
                                hashlib.sha256(candidate.encode("utf-8"))
                                .hexdigest()
                                .encode("utf-8")
                            )
                            value = json.dumps(
                                {"smi": candidate, "gen": "G0.5"}
                            ).encode("utf-8")
                            q.put((key, value))
            ...
            writer.join()
            click.secho(
                f"Found {len(g05_reactions):,} unique G0.5 reactions.", fg="green"
            )
            all_reactions_written.update(g05_reactions)

        click.secho(
            f"\nCombining {len(g0_reactions)} G0 and {len(g05_reactions)} G0.5 reactions for G1.",
            bold=True,
        )
        ...
        if self.require_custom_reactant:
            if not g05_reactions:
                ...
                click.secho(
                    "\n--require-custom-reactant is active. Using ONLY G0.5 reactions for G1.",
                    fg="cyan",
                    bold=True,
                )
            ...
            click.secho(
                f"\nUsing {len(g0_reactions) + len(g05_reactions)} reactions from G0 & G0.5 for G1.",
                bold=True,
            )
        ...
        if self.num_generations >= 1:
            ...
            if os.path.exists(g1_db_path):
                ...
                if len(current_generation_reactions) < 2:
                    click.secho(
                        "Not enough G0 reactions to generate G1. Skipping.", fg="yellow"
                    )
                    g1_reactions = []
                else:
                    reaction_pairs = itertools.combinations(
                        current_generation_reactions, 2
                    )
                    ...
                    with Pool(self.n_workers) as pool:
                        ...
                        for res_list in tqdm(
                            results_iterator, total=total_pairs, desc="G1: Processing"
                        ):
                            for candidate in res_list:
                                if candidate not in all_reactions_written:
                                    all_reactions_written.add(candidate)
        ...
        for gen in range(2, self.num_generations + 1):
            ...
            if os.path.exists(g2plus_db_path):
                ...
                previous_reactions = list(
                    all_reactions_written - set(current_generation_reactions)
                )
                ...
                reaction_pairs = itertools.product(
                    current_generation_reactions, previous_reactions
                )
                total_pairs = len(current_generation_reactions) * len(
                    previous_reactions
                )
                ...
                with Pool(self.n_workers) as pool:
                    ...
                    results_iterator = pool.imap_unordered(
                        partial_worker, reaction_pairs, chunksize=1024
                    )

                    for res_list in tqdm(
                        results_iterator, total=total_pairs, desc=f"G{gen}: Processing"
                    ):
                        for candidate in res_list:
                            if candidate not in all_reactions_written:
                                all_reactions_written.add(candidate)
            ...
            if not next_gen_reactions:
                click.secho(
                    f"No new reactions generated at G{gen}. Stopping.", fg="yellow"
                )
                break
        ...
        click.secho(
            f"Total unique reaction candidates considered: {len(all_reactions_written):,}"
        )
    ...
    def verify_reactions(self):
            ...
            if self.custom_reactants_csv and os.path.exists(self.custom_reactants_csv):
                try:
                    ...
                    if "SMILES" in df_custom.columns:
                        custom_reactants_filter = set(
                            df_custom["SMILES"].dropna().tolist()
                        )
        ...
        writer_env = lmdb.open(
            self._db_paths["verified"], map_size=10**11, writemap=True
        )
        ...
        with writer_env.begin(write=True) as txn_out, Pool(self.n_workers) as pool:
            for verified_batch in tqdm(
                ...
                for result_smi, gen in verified_batch:
                    if result_smi:
                        key = (
                            hashlib.sha256(result_smi.encode("utf-8"))
                            .hexdigest()
                            .encode("utf-8")
                        )
                        value = json.dumps({"smi": result_smi, "gen": gen}).encode(
                            "utf-8"
                        )
    ...
    def export_to_csv(self, filename: str = "reactions.csv"):
        ...
        with env.begin() as txn:
            for key, value in tqdm(
                txn.cursor(), total=num_reactions, desc="Exporting to CSV"
            ):
        ...
        for gen_name, gen_data in reactions_by_gen.items():
            if gen_data:
                gen_output_path = os.path.join(
                    self.output_dir, f"reactions_{gen_name}.csv"
                )
    ...
    def generate_reaction_network_graph(self, filename: str = "reaction_network.json"):
        ...
        with env.begin() as txn:
            for key, _ in tqdm(
                txn.cursor(), total=num_reactions, desc="Building graph"
            ):
                reaction_smarts = key.decode()
    ...
    def export_images(self, image_dir: str = "reaction_images"):
        ...
        with env.begin() as txn:
            for i, (key, _) in enumerate(
                tqdm(
                    txn.cursor(), total=num_reactions, desc="Exporting reaction images"
                )
            ):
                ...
                try:
                    ...
                    logging.warning(
                        f"Could not generate image for reaction {i} ({reaction_smi}). Error: {e}"
                    )
