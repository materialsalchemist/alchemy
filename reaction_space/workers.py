...
def combine_fragments(mol1: Chem.Mol, mol2: Chem.Mol) -> str:
    ...
    try:
        ...
        radical_atoms = [
            a.GetIdx() for a in combo.GetAtoms() if a.GetNumRadicalElectrons() == 1
        ]
        ...
        emol.AddBond(
            radical_atoms[0], radical_atoms[1], order=Chem.rdchem.BondType.SINGLE
        )
    ...
    try:
        ...
        for bond in unsaturated_bonds:
            ...
            for target_atom_idx, other_atom_idx in [
                ...
                original_bond = emol.GetBondBetweenAtoms(
                    target_atom_idx, other_atom_idx
                )
                original_order = original_bond.GetBondType()
                new_order = (
                    Chem.BondType.SINGLE
                    if original_order == Chem.BondType.DOUBLE
...
def worker_generate_higher_gen_reactions(
    reaction_pair: Tuple[str, str], max_reaction_complexity: int = 3
) -> List[str]:
    """
...
def worker_verify_reaction_batch(
    ...
    for i, (reaction_smi, gen) in enumerate(reaction_smi_bytes_batch):
        try:
            ...
            r_cans = sorted(
                [Chem.MolToSmiles(m, canonical=True) for m in reactant_mols]
            )
