...
class ReactionNetwork:
	...
	def save_to_csv_and_images(self, dir_path):
		...
		for i, (reactants, products) in enumerate(sorted(self.reactions)):
			Draw.MolsToImage([Chem.MolFromSmiles(smi) for smi in reactants.split(" + ")]).save(
				f"{dir_path}/reaction_images/reactions_reactants_{i}.png"
			)
			Draw.MolsToImage([Chem.MolFromSmiles(smi) for smi in products.split(" + ")]).save(
				f"{dir_path}/reaction_images/reactions_products_{i}.png"
			)
	...
	def composition_to_mol(self, composition, bond_combo):
		...
		atom_indices = {
			atom: [mol.AddAtom(Chem.Atom(atom)) for _ in range(count)] for atom, count in composition.items()
		}
	...
	def generate_molecules_parallel(self):
		...
		with ThreadPoolExecutor(max_workers=self.n_workers) as executor:
			futures = [executor.submit(self._process_molecule_chunk, chunk) for chunk in chunks]
	...
	def generate_bond_formation(self):
		...
		with ThreadPoolExecutor(max_workers=self.n_workers) as executor:
			futures = [executor.submit(self._process_formation_chunk, chunk) for chunk in chunks]
