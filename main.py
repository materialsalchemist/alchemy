...
class ReactionNetwork:
    ...
    def can_form_bond(
        self, atom1: Chem.Atom, atom2: Chem.Atom, bond_type: Chem.BondType
    ) -> bool:
    ...
    def save_to_csv_and_images(self, dir_path):
        ...
        for i, (reactants, products) in enumerate(sorted(self.reactions)):
            Draw.MolsToImage(
                [Chem.MolFromSmiles(smi) for smi in reactants.split(" + ")]
            ).save(f"{dir_path}/reaction_images/reactions_reactants_{i}.png")
            Draw.MolsToImage(
                [Chem.MolFromSmiles(smi) for smi in products.split(" + ")]
            ).save(f"{dir_path}/reaction_images/reactions_products_{i}.png")
    ...
    def generate_markdown_from_csv_and_images(self, dir_path):
        ...
        for i, (reactants, products) in enumerate(sorted(self.reactions)):
            ...
            markdown_lines.append(
                f"| `{reactants}` | `{products}` | {reactants_md} | {products_md} |"
            )
    ...
    def _deduplicate_reactions(self):
        ...
        self.reactions = set(
            reaction
            for reactant_dict in unique_reactions.values()
            for reaction in reactant_dict.values()
        )
    ...
    def generate_compositions(self):
        ...
        for total_atoms in range(1, self.max_atoms + 1):
            ...
            for atom_counts in itertools.combinations_with_replacement(
                ...
                for bond_combo in itertools.combinations_with_replacement(
                    self.bond_types, num_bonds
                ):
    ...
    def generate_molecules_parallel(self):
        ...
        with ThreadPoolExecutor(max_workers=self.n_workers) as executor:
            futures = [
                executor.submit(self._process_molecule_chunk, chunk) for chunk in chunks
            ]
        ...
        for smiles in smiles_chunk:
            ...
            for bond in mol.GetBonds():
                ...
                try:
                    ...
                    frag_smis = [
                        Chem.MolToSmiles(f, canonical=True, allHsExplicit=True)
                        for f in frags
                    ]
                    if len(frag_smis) == 2:
    ...
    def generate_bond_dissociation(self):
        ...
        chunks = list(
            self._chunk_iterator(
                list(self.molecules),
                max(1, len(self.molecules) // (self.n_workers * 4)),
            )
        )

        with ThreadPoolExecutor(max_workers=self.n_workers) as executor:
            futures = [
                executor.submit(self._process_dissociation_chunk, chunk)
                for chunk in chunks
            ]
    ...
    def _process_formation_chunk(self, mol_pairs_chunk):
        ...
        for mol1_smi, mol2_smi in mol_pairs_chunk:
            ...
            atoms2 = [
                (j, combined.GetAtomWithIdx(j))
                for j in range(num_atoms_mol1, combined.GetNumAtoms())
            ]
            ...
            for (i, atom1), (j, atom2) in itertools.product(atoms1, atoms2):
                ...
                for bond_type in self.bond_types:
                    if not self.can_form_bond(atom1, atom2, bond_type):
                        logging.debug(
                            f"Valence check failed for {atom1_sym}-{atom2_sym} with bond type {bond_type}"
                        )
                        continue
                    ...
                    try:
                        ...
                        product_smi = Chem.MolToSmiles(
                            rwmol, canonical=True, allHsExplicit=True
                        )
                        if self._is_valid_molecule(product_smi):
                            ...
                            logging.debug(
                                f"Successfully created: {reactants} -> {product_smi}"
                            )
                        else:
                            logging.debug(
                                f"Validation failed for product: {product_smi}"
                            )
                    except Exception:
                        continue
    ...
    def generate_transfer_reactions(self):
        ...
        for (react1, prod1), (react2, prod2) in itertools.combinations(
            self.reactions, 2
        ):
    ...
    def save_graph_as_png(self, filename="reaction_network.png"):
        ...
        molecule_nodes = [
            n for n, d in self.graph.nodes(data=True) if d.get("type") == "molecule"
        ]
        reaction_nodes = [
            n for n, d in self.graph.nodes(data=True) if d.get("type") == "reaction"
        ]
...
if __name__ == "__main__":
    ...
    Chem.SanitizeMol(m)
    Draw.MolsToImage([m]).save("test.png" )
    # print(network.heavy_atoms)
    # print(network.generate_compositions_sherif())

    # network.generate_molecules_parallel()
    ...
    # except:
    #     pass
    # network.save_molecules_to_csv_and_images(str(args.max_atoms))
