...
REACTION_NETWORK_JSON_PATH = os.path.join(REACTION_SPACE_RESULTS_PATH, 'reaction_network.json')
...
def load_molecule_data():
	...
	try:
		...
		for col in atom_columns:
			if col in df.columns:
				try:
					...
					app.logger.warning(f"Could not convert column {col} to int. Skipping.")
		...
		app.logger.info(f"Loaded {len(df)} molecules. Heavy atoms: {heavy_atom_columns}")
		return df, smiles_to_image_idx, atom_columns, all_smiles_sorted, heavy_atom_columns
...
def molecules_page():
	...
	if df is None:
		return render_template(
			'molecules.html', error="Molecule data could not be loaded.", molecules=[],
			smiles_to_image_idx={}, atom_columns=[], heavy_atom_columns=[],
			filters={},
		)
	...
	if search_smiles:
		filtered_df = filtered_df[filtered_df['SMILES'].str.contains(search_smiles, case=False, na=False)]
	...
	return render_template(
		'molecules.html', molecules=molecules_list,
